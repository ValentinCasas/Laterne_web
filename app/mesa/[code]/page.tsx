...
/** @summary Valida el QR de una mesa activa antes de vincularlo con la carta del visitante. */
export default async function TableEntryPage({ params }: TableEntryPageProps) {
  const { code } = await params;
  const tenant = await getDefaultTenant();
  const table = await prisma.diningTable.findFirst({ where: { tenantId: tenant.id, code, active: true } });
  if (!table) notFound();
  return (
    <main className="shell grid min-h-[70vh] place-items-center py-12">
      <section className="card max-w-xl p-8 text-center">
        ...
        <TableEntry code={table.code} name={table.name} />
      </section>
    </main>
  );
}
