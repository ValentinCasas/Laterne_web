...
export default async function PlatformBranchDetailPage({
  ...
  const [tenantSlug, branchSlug] = await Promise.all([
    (await params).tenantSlug,
    (await params).branchSlug,
  ]);
  const tenant = await prisma.tenant.findUnique({ where: { slug: tenantSlug.trim().toLocaleLowerCase("es") }, select: { id: true, name: true, slug: true, status: true } });
  if (!tenant) notFound();

  const branch = await prisma.branch.findFirst({
    where: { tenantId: tenant.id, slug: branchSlug.trim().toLocaleLowerCase("es") },
    include: {
      licenses: { include: { plan: { select: { id: true, name: true, slug: true } } }, orderBy: { id: "desc" } },
      membershipAccess: {
        include: {
          membership: { include: { user: { select: { name: true, email: true } }, role: { select: { name: true, key: true } } } },
        },
        orderBy: { id: "asc" },
      },
  ...
  const plans = await prisma.plan.findMany({ where: { active: true }, select: { id: true, name: true }, orderBy: { displayOrder: "asc" } });
