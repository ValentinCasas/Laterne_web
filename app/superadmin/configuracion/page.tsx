...
/**
 * @summary Carga la configuración visual de la plataforma.
 */
export default async function PlatformConfigurationPage() {
  ...
  const palettes = await prisma.platformPalette.findMany({
    where: { settingsId: 1 },
    ...
    <PlatformAppearance
      initialSettings={
        serialize({
          ...
          isotypeUrl: settings.isotypeUrl,
      ...
      initialPalettes={
        serialize(palettes) as unknown as Parameters<typeof PlatformAppearance>[0]["initialPalettes"]
      }
      presets={menuClickPresets as (PaletteColors & { key: string; name: string; description: string })[]}
    />
  );
}
