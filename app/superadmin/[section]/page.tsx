...
const labels: Record<string, string> = {
  ...
  dominios: "Dominios",
  uso: "Uso y límites",
  soporte: "Soporte SaaS",
  auditoria: "Auditoría global",
  configuracion: "Configuración MenuClick",
};
type PlatformRow = { title: string; detail: string; status?: string; href?: string };

/**
 * @summary Carga una sección global de supervisión de la plataforma.
 */
export default async function PlatformSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "auditoria") {
    ...
    rows = logs.map((log) => ({
      title: `${log.tenant?.name ?? "Global"} · ${log.action}`,
      detail: `${log.entityType} · ${new Date(log.createdAt).toLocaleString("es-AR")}`,
      status: log.result,
    }));
  }
  return (
    <section>
      <header className="mb-7 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-4xl font-black">{labels[section]}</h1>
          <p className="mt-3 text-slate-400">Supervisión global con datos separados por cliente.</p>
        </div>
        {section === "configuracion" && (
          <Link
            className="rounded-xl bg-amber-400 px-4 py-3 text-sm font-black text-slate-950"
            href="/platform/planes"
          >
      ...
      {section === "configuracion" ? (
        <div className="grid gap-4 md:grid-cols-3">
          <Link
            ...
            href="/platform/planes"
          >
            <strong className="text-xl">Catálogo de planes</strong>
            <p className="mt-2 text-sm text-slate-400">Precios, capacidades y funcionalidades incluidas.</p>
          </Link>
          <Link
            className="rounded-2xl border border-white/10 bg-[#151a24] p-5 hover:border-amber-300/40"
            href="/platform/auditoria"
          >
            <strong className="text-xl">Política de auditoría</strong>
            <p className="mt-2 text-sm text-slate-400">Revisá acciones sensibles de toda la plataforma.</p>
          </Link>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#151a24]">
          <div className="hidden grid-cols-[minmax(220px,1fr)_minmax(220px,2fr)_150px] gap-4 border-b border-white/10 px-5 py-4 text-xs font-black uppercase tracking-wider text-slate-500 sm:grid">
          ...
          {rows.map((row, index) => (
            <a
              className="grid gap-2 border-b border-white/10 px-5 py-4 hover:bg-white/[.04] sm:grid-cols-[minmax(220px,1fr)_minmax(220px,2fr)_150px] sm:gap-4"
              href={row.href}
              key={`${row.title}-${index}`}
            >
              <span className="font-black">{row.title}</span>
              <span className="text-sm text-slate-400">{row.detail}</span>
              <span className="text-sm text-amber-200">{row.status ?? "-"}</span>
            </a>
          ))}
          {!rows.length && <p className="p-12 text-center text-slate-400">No hay registros para mostrar.</p>}
        </div>
      )}
    </section>
  );
