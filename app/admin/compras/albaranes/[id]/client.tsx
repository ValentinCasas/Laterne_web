 ...
 */
export function ComprasAlbaranDetailClient({
  receipt,
  currency,
}: {
  receipt: AlbaranDetail;
  currency: string;
}) {
  const pathname = usePathname();
  const href = (path: string) => adminHrefFromPathname(pathname, path);
  const totalReceipt = receipt.items.reduce(
      ...
      >
        <div className="mx-auto max-w-[1600px] flex flex-wrap items-center px-4 py-2 text-xs gap-0 sm:px-8 sm:py-2 sm:text-xs">
          {receipt.order && (
            ...
            >
              <span
                className="mr-2 text-[9px] font-bold uppercase tracking-wider"
              ...
              <Link
                ...
                className="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold transition-all duration-150"
                style={{ color: "var(--admin-muted)" }}
                onMouseEnter={(e) => {
              ...
              {receipt.invoices.map(({ invoice }) => (
                <Link
                  key={invoice.id}
                  href={href(`/admin/compras/facturas/${invoice.id}`) as never}
                  className="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold transition-all duration-150 ml-1"
                  style={{ color: "var(--admin-muted)" }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background =
                      "color-mix(in srgb, var(--admin-primary) 8%, transparent)";
          ...
          <span className="px-3 text-[10px] italic" style={{ color: "var(--admin-muted)" }}>
          ...
          >
            ...
            >
              <h3 className="text-sm font-bold" style={{ color: "var(--admin-text)" }}>
                Lineas{" "}
                <span className="text-[10px] font-normal ml-1" style={{ color: "var(--admin-muted)" }}>
                          ...
                          <td className="px-4 py-3 text-sm sm:px-5 sm:py-3">
                            {item.orderItem?.order ? (
                              <Link
                                href={
                                  href(
                                    `/admin/compras/pedidos/${item.orderItem.order.id}?line=${item.orderItem.id}`,
                                  ) as never
                                }
                                ...
                                onMouseLeave={(e) => (e.currentTarget.style.opacity = "1")}
                              >
                                {item.orderItem.order.number} /{" "}
                                {String(
                                  item.orderItem.sortOrder ? (item.orderItem.sortOrder + 1) * 10000 : 10000,
                                ).padStart(5, "0")}
                              </Link>
                            ) : (
          ...
          {receipt.notes && (
            <div className="overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-xl shadow-black/10">
              ...
              <div className="px-6 py-5">
                <p className="text-sm whitespace-pre-wrap" style={{ color: "var(--admin-muted)" }}>
                  {receipt.notes}
                </p>
              </div>
            </div>
