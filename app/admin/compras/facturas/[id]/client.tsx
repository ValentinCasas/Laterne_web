...
const INVOICE_STATUS: Record<string, { label: string; color: string; bg: string }> = {
  draft: {
    ...
    bg: "color-mix(in srgb, var(--admin-muted) 12%, transparent)",
  },
  pending: {
    label: "Pendiente",
  ...
  cancelled: {
    ...
    bg: "color-mix(in srgb, var(--admin-danger) 12%, transparent)",
  },
};
      ...
      <div style={{ background: "var(--admin-surface)" }} className="relative">
        ...
        <div className="mx-auto max-w-[1600px] px-8 pt-6 pb-5">
          <nav className="mb-5 flex items-center gap-2 text-xs" style={{ color: "var(--admin-muted)" }}>
            ...
            <Link href={href("/admin/compras/facturas")} className="transition-colors hover:opacity-70">
              Facturas
            </Link>
            <span className="opacity-40">/</span>
          ...
          <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
            <div>
              ...
              <p className="mt-2 text-sm" style={{ color: "var(--admin-muted)" }}>
                {invoice.supplier.name} · {dateLabel(invoice.documentDate)}
              </p>
            </div>
          ...
          <ActionGroup label="Pedido">
            {invoice.order && (
              <>
                ...
                <Link
                  href={href(`/admin/compras/pedidos/${invoice.order.id}`) as never}
                  className="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold transition-all whitespace-nowrap flex items-center gap-1.5 hover:opacity-80"
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          >
            <div className="grid gap-x-10 gap-y-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <FactField
                label="Pagado"
                value={money(invoice.paidAmount, currency)}
                color="var(--admin-success)"
              />
              <FactField
                label="Saldo"
                value={money(balance, currency)}
                color={balance > 0 ? "var(--admin-warning)" : "var(--admin-success)"}
                bold
              />
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-xl shadow-black/10">
            <div className="px-5 py-3 border-b" style={{ borderColor: "var(--admin-border)" }}>
              <h3
                ...
                style={{ color: "var(--admin-muted)" }}
              >
                Lineas de la factura ({invoice.items.length})
              </h3>
            </div>
            <div className="overflow-x-auto" style={{ scrollbarColor: "var(--admin-border) transparent" }}>
              <table className="w-full text-left text-xs">
                <thead>
                  ...
                  >
                    <th className="px-4 py-2.5 font-semibold" style={{ color: "var(--admin-muted)" }}>
                      #
                    </th>
                    <th className="px-4 py-2.5 font-semibold" style={{ color: "var(--admin-muted)" }}>
                      Concepto
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          background:
                            idx % 2 === 1
                              ? "color-mix(in srgb, var(--admin-surface-elevated) 12%, var(--admin-surface))"
                              : undefined,
                        }}
                      >
                        <td className="px-4 py-2 text-[10px]" style={{ color: "var(--admin-muted)" }}>
                          {String((idx + 1) * 10000).padStart(5, "0")}
                        </td>
                        <td className="px-4 py-2 font-semibold" style={{ color: "var(--admin-text)" }}>
                          {item.description}
                        </td>
                        <td
                ...
                <tfoot>
                  <tr
                    style={{
                      ...
                      background:
                        "color-mix(in srgb, var(--admin-surface-elevated) 25%, var(--admin-surface))",
                    }}
                  >
                    <td
                    ...
                    <td
                      className="px-4 py-2.5 text-right font-bold tabular-nums text-xs"
                      style={{ color: "var(--admin-text)" }}
                    >
                  ...
                  <tr style={{ borderBottom: "1px solid var(--admin-border)" }}>
                    <td className="px-4 py-2.5 text-xs" colSpan={4} style={{ color: "var(--admin-muted)" }}>
                      Impuestos
                    </td>
                    <td
                  ...
                  <tr
                    style={{
                      background:
                        "color-mix(in srgb, var(--admin-surface-elevated) 40%, var(--admin-surface))",
                    }}
                  >
                    <td
                    ...
                    <td
                      className="px-4 py-2.5 text-right font-extrabold tabular-nums text-xs"
                      style={{ color: "var(--admin-text)" }}
                    >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr
                      className="text-[10px] uppercase tracking-wider"
                      style={{ borderBottom: "1px solid var(--admin-border)", color: "var(--admin-muted)" }}
                    >
                      <th className="px-4 py-2 font-semibold" style={{ color: "var(--admin-muted)" }}>
                        Albaran
                      </th>
                      <th className="px-4 py-2 font-semibold" style={{ color: "var(--admin-muted)" }}>
                        Fecha
                  ...
                  <tbody>
                    {invoice.receipts.map(({ receipt }, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2" style={{ color: "var(--admin-muted)" }}>
                          {dateLabel(receipt.receivedAt)}
                        </td>
                        <td className="px-4 py-2">
                          {receipt.order ? (
                            <Link
                              href={href(`/admin/compras/pedidos/${receipt.order.id}`) as never}
                              className="font-bold transition-opacity hover:opacity-80"
                              style={{ color: "var(--admin-primary)" }}
                            >
                        ...
                        <td className="px-4 py-2 text-right" style={{ color: "var(--admin-muted)" }}>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-xl shadow-black/10">
            ...
            <div className="p-4">
              {invoice.payments.length === 0 ? (
                <p className="text-xs py-4 text-center" style={{ color: "var(--admin-muted)" }}>
                ...
                <div className="space-y-2">
                  {invoice.payments.map((payment) => (
                    <div
                      key={payment.id}
                      className="flex flex-wrap items-center gap-3 rounded-lg px-4 py-2.5 text-xs"
                      style={{
                        background:
                          "color-mix(in srgb, var(--admin-surface-elevated) 40%, var(--admin-surface))",
          ...
          {canPay && (
            <div
              ...
              style={{
                ...
                background: "color-mix(in srgb, var(--admin-warning) 4%, transparent)",
              }}
            >
              <p className="text-xs font-bold mb-3" style={{ color: "var(--admin-warning)" }}>
                Registrar pago
              </p>
              ...
              <div className="mt-3 flex items-center gap-3">
                <button
                  type="button"
                  className="rounded-lg px-4 py-1.5 text-xs font-bold text-white transition-all hover:opacity-90"
                ...
                <span className="text-[10px]" style={{ color: "var(--admin-muted)" }}>
                  Saldo pendiente: {money(balance, currency)}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="w-full lg:w-72 shrink-0 space-y-4">
          ...
          >
            ...
            <FactBoxSection title="Proveedor">
              ...
              {invoice.supplier.paymentTerms && (
                <FactBoxRow label="Pago" value={invoice.supplier.paymentTerms} />
              )}
            </FactBoxSection>
            <FactBoxSection title="Documentos relacionados">
              ...
              {invoice.receipts.length > 0 && (
                <div className="space-y-1 mt-2">
                  <span className="text-[10px] font-semibold" style={{ color: "var(--admin-muted)" }}>
                    Albaranes ({invoice.receipts.length})
                  </span>
                  {invoice.receipts.map(({ receipt }) => (
                    <Link
                      key={receipt.id}
                      href={href(`/admin/compras/albaranes/${receipt.id}`) as never}
                      className="block text-[10px] font-semibold transition-opacity hover:opacity-80"
                      style={{ color: "var(--admin-primary)" }}
                    >
                      {receipt.number}
                    </Link>
...
function OrderQuickView({
  ...
  const [order, setOrder] = useState<PurchaseOrderPreview | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/admin/compras/${orderId}`, {
          headers: { "Content-Type": "application/json" },
        });
        if (r.ok) {
          const data = (await r.json()) as PurchaseOrderPreview;
    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold" style={{ color: "var(--admin-text)" }}>
                {loading ? "Cargando..." : (order?.number ?? orderNumber)}
              </h2>
              {order && (
                <p className="text-xs mt-0.5" style={{ color: "var(--admin-muted)" }}>
                  {order.supplier?.name} · {ORDER_LABELS[order.status] ?? order.status}
                </p>
              )}
            </div>
            <button
        ...
        <div className="overflow-y-auto p-6 space-y-4" style={{ maxHeight: "calc(80vh - 70px)" }}>
          {loading ? (
            <div className="py-12 text-center text-xs" style={{ color: "var(--admin-muted)" }}>
              Cargando pedido...
            </div>
          ) : order ? (
            <>
              ...
              <div className="grid gap-3 sm:grid-cols-3 text-xs">
                ...
                <QVField
                  label="Fecha"
                  value={order.orderDate ? new Date(order.orderDate).toLocaleDateString("es-AR") : "—"}
                />
                <QVField label="Estado" value={ORDER_LABELS[order.status] ?? order.status} />
                <QVField label="Sucursal" value={order.branch?.name ?? "—"} />
                <QVField
                  label="Recepcion prevista"
                  value={order.expectedDate ? new Date(order.expectedDate).toLocaleDateString("es-AR") : "—"}
                />
                  ...
                  >
                    <table className="w-full text-left text-[10px]">
                      <thead>
                        ...
                        >
                          <th className="px-3 py-2 font-semibold" style={{ color: "var(--admin-muted)" }}>
                            Producto
                          </th>
                          <th
                      ...
                      <tbody>
                        {order.items.map((item, idx) => (
                          ...
                          >
                            <td className="px-3 py-2 font-semibold" style={{ color: "var(--admin-text)" }}>
                              {item.product?.name ?? "—"}
                            </td>
                            <td
              ...
              >
                <span className="text-xs font-bold tabular-nums" style={{ color: "var(--admin-text)" }}>
                  Total:{" "}
                  {money(
                    order.items?.reduce(
                ...
                <Link
                  href={href(`/admin/compras/pedidos/${order.id}`) as never}
                  className="rounded-lg px-4 py-1.5 text-xs font-bold text-white transition-all hover:opacity-90"
                  style={{ background: "var(--admin-primary-strong)" }}
                  onClick={onClose}
                >
...
function ActionGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div
      className="flex items-center border-r px-3 py-1.5 last:border-r-0"
      style={{ borderColor: "color-mix(in srgb, var(--admin-border) 60%, transparent)" }}
    >
      <span
        className="mr-2 text-[9px] font-bold uppercase tracking-wider whitespace-nowrap"
        style={{ color: "var(--admin-muted)" }}
      >
        {label}
      </span>
...
function FactField({
  ...
  return (
    <div>
      ...
      <p
        className={`text-sm ${bold ? "font-extrabold" : "font-bold"}`}
