...
import { PageHeader, SectionHeader, KpiCard, EmptyState } from "@/components/admin/ui";
...
export default async function ResumenPage({ searchParams }: ResumenPageProps) {
  ...
  return (
    ...
    >
      <PageHeader eyebrow="Reportes" title="Resumen" description="KPIs generales y evolución del negocio." section="reportes" />
      ...
      <section className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 sm:p-7">
        <SectionHeader title="Top productos" description="Los 5 productos más vendidos en el período." />
        {topProducts.ranking.length === 0 ? (
          <EmptyState title="No hay ventas para este período." description="Probá ampliar el rango de fechas o cambiar la sucursal." />
        ) : (
          <div className="mt-4 space-y-3">
            {topProducts.ranking.map((product, index) => (
              <article key={product.productId} className="grid grid-cols-[32px_1fr_auto] items-center gap-3 rounded-2xl bg-white/5 p-3 transition-colors hover:bg-white/[0.07]">
                <span className="text-center text-sm font-black text-pink-300">{index + 1}</span>
                <div>
                  <strong className="text-sm font-bold text-zinc-200">{product.productName}</strong>
                  <p className="text-xs text-zinc-500">{product.units} unidades</p>
                </div>
                <span className="text-sm font-black tabular-nums text-zinc-300">{product.sales.toLocaleString("es-AR")}</span>
              </article>
            ))}
          </div>
        )}
