...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: `${context.tenant.name} | Variantes y agregados` };
}
...
export default async function ProductOptionsPage() {
  ...
  const optionProductWhere =
    context.activeBranchId && context.activeBranchId > 0
      ? { product: { branchAssignments: { some: { branchId: context.activeBranchId } } } }
      : {};
  ...
  return (
    <ProductOptionsManager
      ...
      initialGroups={
        serialize(groups) as unknown as Parameters<typeof ProductOptionsManager>[0]["initialGroups"]
      }
    />
  );
}
