...
export default async function BranchLandingPage({ params }: { params: Promise<{ branchSlug: string }> }) {
  ...
  const ownLanding =
    !branch.inheritLanding &&
    branch.branch.landingContent &&
    typeof branch.branch.landingContent === "object" &&
    !Array.isArray(branch.branch.landingContent)
      ? (branch.branch.landingContent as { heroTitle?: unknown; heroSubtitle?: unknown })
      : null;
  const heroTitle =
    typeof ownLanding?.heroTitle === "string" && ownLanding.heroTitle.trim()
      ? ownLanding.heroTitle
      : typeof brand?.heroTitle === "string" && brand.heroTitle.trim()
        ? brand.heroTitle
        : `${tenant.name} es`;
  const heroSubtitle =
    typeof ownLanding?.heroSubtitle === "string" && ownLanding.heroSubtitle.trim()
  ...
  const branchLatitude = Number.isFinite(Number(branchInfo?.latitude))
    ? Number(branchInfo?.latitude)
    : Number.isFinite(Number(business?.latitude))
      ? Number(business?.latitude)
      : null;
  const branchLongitude = Number.isFinite(Number(branchInfo?.longitude))
