...
import { payPurchaseInvoice } from "@/lib/purchases";
...
/** @summary Registra un pago parcial o total contra la factura. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await payPurchaseInvoice(auth.tenant.id, auth.session.userId, {
      invoiceId: Number(id),
