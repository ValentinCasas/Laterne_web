...
export async function POST(
  ...
  if (!superAdmin?.user.isSuperAdmin || process.env.NODE_ENV !== "development")
    return NextResponse.json(
      { error: "Esta acción solo está disponible para superadmin en desarrollo" },
      { status: 403 },
    );
  const params = await context.params;
  const tenantId = Number(params.tenantId);
  const userId = Number(params.userId);
  if (!Number.isInteger(tenantId) || !Number.isInteger(userId))
  ...
  if (!membership)
    return NextResponse.json({ error: "Usuario no encontrado en este cliente" }, { status: 404 });
  ...
  await recordAudit({
    ...
    entityType: "user",
    entityId: userId,
    newValues: { developmentOnly: true, sessionsRevoked: true },
    request,
  });
