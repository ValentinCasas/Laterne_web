...
/**
 * @summary Actualiza la configuración tras validar contexto, permisos y entrada.
 */
export async function PATCH(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "Revisá la identidad de MenuClick" }, { status: 400 });
  ...
  const settings = await prisma.platformSettings.upsert({
    ...
    update: {
      ...
      isotypeUrl: parsed.data.isotypeUrl || null,
  ...
  await recordAudit({
    ...
    entityId: 1,
    oldValues: current ? toAuditValue(current) : undefined,
