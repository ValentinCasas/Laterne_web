...
const password = `full-cycle-${Date.now()}`;
const email = `full-cycle-${Date.now()}@example.com`;
...
test("Cliente, entrega y cobros cargan sin errores", async ({ page }) => {
  const errors: string[] = [];
  page.on("console", (msg) => {
    if (msg.type() === "error") errors.push(msg.text());
  });

  const clientesResponse = await page.goto(`/t/${tenantSlug}/admin/clientes`);
  if (!clientesResponse) throw new Error("No response");
