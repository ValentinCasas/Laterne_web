...
export function CopyTrackingLink({ href, compact = false }: { href: string; compact?: boolean }) {
  ...
  if (compact) {
    return (
      <button className="btn btn-secondary flex items-center gap-1.5" onClick={() => void copy()} type="button">
  ...
  return (
    <section className="border-y border-white/10 bg-white/[.03] p-5 sm:p-8">
      ...
      <button className="btn mt-3 flex min-h-11 items-center gap-1.5" onClick={() => void copy()} type="button">
        {copied ? <Icon name="check" className="h-4 w-4" /> : <Icon name="copy" className="h-4 w-4" />}
