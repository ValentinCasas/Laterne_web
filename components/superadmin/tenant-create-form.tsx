...
export function TenantCreateForm({ plans }: { plans: Array<{ id: number; name: string }> }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/superadmin/tenants", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget).entries())) });
    ...
    router.push(result.tenantId ? `/superadmin/clientes/${result.tenantId}` : "/superadmin/clientes");
    router.refresh();
  }
