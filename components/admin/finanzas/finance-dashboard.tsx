...
export function FinanceDashboardClient({ initial }: { initial: DashboardInitial }) {
  ...
  const kpis = useMemo(
    () => [
      { label: "Saldo total", value: dashboard.totalBalance, tone: "text-emerald-300" },
      { label: "Vencido cobrar", value: dashboard.receivablesOverdue, tone: "text-rose-300" },
      { label: "Vencido pagar", value: dashboard.payablesOverdue, tone: "text-red-300" },
      { label: "Resultado operativo", value: dashboard.operatingResult, tone: "text-violet-300" },
    ],
    [dashboard],
  );
  ...
  const detailRows = useMemo(
    () => [
      ...
      { label: "Bancos", value: money(dashboard.bankBalance, currency), tone: "text-indigo-300" },
      {
        ...
        value: money(dashboard.receivablesTotal, currency),
        tone: "text-amber-300",
      },
  ...
  const tableData = useMemo(
    () =>
      dashboard.recentMovements.map((movement) => ({
        ...
        importe: (
          <span
            className={`text-right font-black tabular-nums block ${movement.direction === "in" ? "text-emerald-300" : "text-rose-300"}`}
          >
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="lg:col-span-1">
          <div className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4">
            <h3 className="mb-3 text-sm font-black uppercase tracking-wider text-[var(--admin-muted)]">
              Detalle
            </h3>
        ...
        <div className="lg:col-span-2">
          <div className="overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-xl shadow-black/10">
            <div className="px-5 py-4">
              <h2 className="text-sm font-black uppercase tracking-wider text-[var(--admin-muted)]">
