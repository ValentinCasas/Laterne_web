...
export function DocumentHeader({ reference, title, status, actions, children }: { reference: string; title: string; status?: ReactNode; actions?: ReactNode; children?: ReactNode }) {
  return (
    <header className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-widest text-pink-300">{reference}</p>
          <h1 className="mt-1.5 text-xl font-black tracking-tight sm:text-2xl">{title}</h1>
