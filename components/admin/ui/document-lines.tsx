  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-[var(--admin-shadow-sm)]">
      <table className="w-full text-left">
        <thead>
          <tr
            className={`border-b border-[var(--admin-border)] bg-[var(--admin-surface-elevated)] uppercase tracking-wider text-[var(--admin-muted)] ${headerClass}`}
          >
            {headers.map((header, index) => (
              <th key={index} className="font-bold first:pl-5 last:pr-5">
                {header}
              </th>
            ))}
          </tr>
