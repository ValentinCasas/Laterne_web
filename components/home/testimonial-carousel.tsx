...
/** @summary Presenta opiniones aprobadas en un carrusel con altura controlada y modal «Ver completo». */
export function TestimonialCarousel({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  const openTestimonial = testimonials.find((item) => item.id === openId) ?? null;
    ...
    <div role="region" aria-label="Opiniones de clientes" aria-roledescription="carrusel">
      ...
      >
        {testimonials.map((item) => (
          <article
            className="relative h-72 min-w-full snap-start rounded-3xl border border-pink-500/35 bg-gradient-to-b from-pink-950/55 to-zinc-950 p-6 pt-16 text-center shadow-2xl shadow-black/30 md:min-w-[calc((100%_-_1.25rem)/2)]"
            key={item.id}
          >
            ...
            <blockquote className="mt-4 line-clamp-5 text-base leading-relaxed text-zinc-100">
              <span aria-hidden="true">“</span>
              {item.description}
              <span aria-hidden="true">”</span>
            </blockquote>
            {item.description.length > TRUNCATE_AT && (
              <button
                className="absolute inset-x-0 bottom-3 text-sm font-bold text-pink-400 hover:text-pink-300"
      ...
      {openTestimonial && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Opinión completa"
          onClick={() => setOpenId(null)}
        >
          <div
            className="max-h-[85dvh] w-full max-w-lg overflow-y-auto rounded-3xl border border-pink-500/35 bg-zinc-950 p-7 text-center shadow-2xl shadow-black/40"
            onClick={(event) => event.stopPropagation()}
          >
            <figure className="mx-auto h-20 w-20 rounded-full bg-gradient-to-br from-pink-500 to-orange-400 p-1">
              <Image
                src={`/images/avatars_defect/${openTestimonial.avatar}`}
                alt="Cliente del negocio"
                fill
                sizes="80px"
                className="pointer-events-none rounded-full border-4 border-zinc-950 object-cover p-1"
            ...
            <time className="mt-4 block text-sm font-bold text-pink-400">{openTestimonial.date}</time>
            <blockquote className="mt-4 text-lg leading-relaxed text-zinc-100">
              <span aria-hidden="true">“</span>
              {openTestimonial.description}
              <span aria-hidden="true">”</span>
            </blockquote>
            <button className="btn btn-secondary mt-7" onClick={() => setOpenId(null)} type="button">
              Cerrar
            </button>
          </div>
        </div>
