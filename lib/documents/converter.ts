...
/**
 * @summary Comprueba que una conversión remota devolvió un archivo PDF válido.
 */
...
/**
 * @summary Construye el formulario multipart enviado al conversor documental.
 */
...
/**
 * @summary Convierte documentos mediante un servicio HTTP configurado.
 */
class RemoteDocumentConverter implements DocumentConverter {
  ...
  /**
   * @summary Envía un DOCX al servicio remoto y valida el PDF recibido.
   */
    ...
    try {
      ...
      if (!response.ok) {
        return {
          status: "error",
          converter: "remote",
          message: `El conversor remoto respondió ${response.status}.`,
        };
      }
      ...
      if (pdf.byteLength > 20 * 1024 * 1024 || !pdfBytesAreValid(pdf)) {
        return {
          status: "error",
          converter: "remote",
          message: "El conversor remoto no devolvió un PDF válido.",
        };
      }
      return {
        status: "ready",
        converter: "remote",
        pdf,
        message: "PDF generado por el servicio configurado.",
      };
    } catch (error) {
      const message =
...
/**
 * @summary Enumera las ubicaciones conocidas de LibreOffice según el sistema operativo.
 */
...
/**
 * @summary Comprueba si un ejecutable puede utilizarse como conversor local.
 */
...
/**
 * @summary Convierte documentos con una instalación local de LibreOffice.
 */
class LibreOfficeDocumentConverter implements DocumentConverter {
  /**
   * @summary Convierte un DOCX a PDF con una instalación local de LibreOffice.
   */
  async convert(docx: Uint8Array): Promise<DocumentConversionResult> {
      ...
      for (const command of libreOfficeCandidates()) {
        ...
        try {
          ...
          return {
            ...
            pdf,
            message: "PDF generado con LibreOffice headless.",
          };
        } catch (error) {
          ...
          return {
            ...
            converter: "libreoffice",
            message:
              "LibreOffice no pudo convertir el DOCX. El archivo Word sigue disponible para descargar.",
          };
        }
      }
      return {
        ...
        message:
          "LibreOffice no está instalado. Podés descargar el DOCX; configurá LIBREOFFICE_PATH o DOCUMENT_CONVERTER_URL para obtener PDF.",
      };
    } finally {
