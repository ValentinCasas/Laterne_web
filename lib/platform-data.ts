...
export async function platformTenants() {
  ...
  return tenants.map((tenant) => ({ ...serialize(tenant), publicUrl: publicTenantUrl(tenant.slug, tenant.brandSettings?.customDomain), adminUrl: tenantAdminUrl(tenant.slug, "/admin"), storageBytes: Number(storageByTenant.get(tenant.id) ?? 0) }));
}
