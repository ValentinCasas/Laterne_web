...
async function main() {
  const collation = await prisma.$queryRawUnsafe<{ collation: string }[]>("SHOW VARIABLES LIKE 'collation_database'");
  const dbCollation = collation[0]?.collation || "utf8mb4_general_ci";
  ...
  const sql = `
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=${dbCollation};
